...
int main(){
    ...
    Test(){
        ...
        ll min_even=a[0];
        for(ll i=0;i<n;i+=2){
            min_even=__gcd(min_even,a[i]);
        }
        bool check=true;
        FOR(i,0,n-1){
            if(a[i]%min_even==0&&a[i+1]%min_even!=0||
               a[i]%min_even!=0&&a[i+1]%min_even==0)
               continue;
            else{
                check=false;
                break;
        ...
        if(check)
            ...
            ll min_odd=a[1];
            for(ll i=1;i<n;i+=2){
                min_odd=__gcd(min_odd,a[i]);
            }
            FOR(i,0,n-1){
                if(a[i]%min_odd==0&&a[i+1]%min_odd!=0||
                a[i]%min_odd!=0&&a[i+1]%min_odd==0)
                    continue;
                else{
                    check=false;
                    break;
