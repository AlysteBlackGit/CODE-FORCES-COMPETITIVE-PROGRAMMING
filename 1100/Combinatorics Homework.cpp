...
int main(){
    ...
    Test(){
        ...
        cin>>p;
        sort(all(a));
        ll max_p=a[0]+a[1]+a[2]-3;
        ll min_p=a[2]-a[1]-a[0]-1;
        // if(a[0]+a[1]<a[2]-1) min_p=a[2]-a[0]-a[1];
        // else if(a[0]+a[1]<=a[2]+1) min_p=0;
        // else min_p=a[0]+a[1]-a[2];
        (p<=max_p&&p>=min_p)?(cout<<"YES"<<endl):(cout<<"NO"<<endl); 
    }
}
